...
static int getString(char *pBuffer, int limite)
{
    ...
    {
        myFpurge();
        if(fgets(bufferString, 4096, stdin))
        {
...
*
*/
int utn_isValidFloatPositivo(char *pBuffer, int limite)
{
    int retorno = 0;
    int i;
                            ...
                            char *mensajeError, int reintentos)
{
        ...
        mensajeError != NULL && reintentos>=0)
    {
        do
        {
            ...
            printf("\n%s", mensaje);
            if( getString(bufferFloat, limite) == 0 &&
                utn_isValidFloatPositivo(bufferFloat, limite))
            {
                *pFloat = atof(bufferFloat);
                retorno = 0;
                break;
...
int utn_minimoDesdeEnArrayEnteros(int *pArray, int limite, int desde, int *pMinimo)
{
    int retorno=-1;
    int i;
    int menor = pArray[desde];
    int auxiliarIndiceMinimo = desde;
    if(pArray != NULL && limite > 0 && limite >= desde)
    {
        for(i=desde + 1; i<limite; i++)
        {
            if(pArray[i] < menor)
            {
                menor = pArray[i];
